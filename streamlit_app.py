...
with tab1:
    ...
    today_label = today_kst.strftime("%Y년 %m월 %d일")

    st.info(f"📅 오늘 날짜: **{today_label}**")
    
    # 세션 상태 초기화
    if "meal_data" not in st.session_state:
        st.session_state.meal_data = None

    if "meal_loaded_date" not in st.session_state:
        st.session_state.meal_loaded_date = None

    # 앱이 처음 열렸거나 날짜가 바뀐 경우 자동으로 급식 불러오기
    today_key = today_kst.strftime("%Y%m%d")

    if st.session_state.meal_loaded_date != today_key:
        with st.spinner("오늘 급식 정보를 자동으로 불러오는 중입니다..."):
            st.session_state.meal_data = get_school_meal()
            st.session_state.meal_loaded_date = today_key

    # 급식 불러오기 결과 표시
    if st.session_state.meal_data:
        st.success("✅ 오늘 급식 정보를 불러왔습니다.")
    else:
        st.warning("⚠️ 오늘 급식 정보를 자동으로 불러오지 못했습니다. 아래 입력창에 직접 입력할 수 있습니다.")
